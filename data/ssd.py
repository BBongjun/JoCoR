    ...
    for disk_id, group in tqdm(data.groupby("disk_id")):
        ...
        if group['label'].sum() > 0:
            ...
            if group.shape[0] >= 121:
                ...
                for i in start_list:
                    sequence_features = group.iloc[i:i + window_size][FEATURE_COLUMNS] # (90,27)
                    sequence_features = sequence_features.to_numpy().transpose(1,0)  #(27,90)
                    label = group.iloc[i+window_size, -2]
                    noise = group.iloc[i+window_size, -1]
...
def load_ssd_dataset(args):
    ...
    test_data, test_label, _ = create_sequences(test_scaled, FEATURE_COLUMNS, window_size, number)
    train_label = np.where(train_noise_mask == 1, 1 - noise_label, noise_label)
    
    del train_scaled
    
    print('Num of train :', train_data.shape[0])
    print('Num of test :', test_data.shape[0])
    # test_scaled 는 마지막 final_test에서 필요해서 return해주는거임!
    return train_data, train_label, noise_label, test_data, test_label, test_scaled
...
class ssd_dataset(Dataset):
    def __init__(self, data, real_label, label, mode):
        ...
        self.noise_or_not = real_label == label #noise_mask
...
class ssd_dataloader():
    def __init__(self, dataset, r, batch_size, num_workers, root_dir, args, noise_file=''):
